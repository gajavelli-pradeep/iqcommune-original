...
import { AlreadyRecordedError, recordRating } from "@/services/link-writes";
...
export async function POST(request: Request) {
  ...
  try {
    const { allowed, enforced } = await checkRateLimit(`rating:${clientIdentifier(request)}`);
    ...
    const receipt = await recordRating(token.payload.id, parsed.data.rating, parsed.data.comments);
