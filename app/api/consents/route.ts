...
import { AlreadyRecordedError, recordConsent } from "@/services/link-writes";
...
export async function POST(request: Request) {
  ...
  try {
    const { allowed, enforced } = await checkRateLimit(`consent:${clientIdentifier(request)}`);
    ...
    const receipt = await recordConsent(token.payload.id);
