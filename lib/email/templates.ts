...
function row(label: string, value: string) {
  return `<tr><td style="${TABLE_CELL_L}">${label}</td><td style="${TABLE_CELL_R}">${value}</td></tr>`;
}
...
export function availabilityCheckEmail(
  ...
  const first = practitionerName.split(" ")[0];
  return {
...
<div style="text-align:center;margin:2rem 0">
  <a href="${onboardingUrl}" style="${GOLD_BTN}">Review &amp; Sign Agreement →</a>
</div>
...
export function clientFollowUpEmail(
  ...
  return {
    ...
    htmlContent: `<div style="${BASE}">
<p>Hi ${first},</p>
<p>Thank you for your interest in iqcommune. We've reviewed your request for a session on <strong>${request.topic}</strong>.</p>
<p>We're aligning a practitioner for your group and will confirm session details within 2–3 working days.</p>
<p style="font-size:13px;color:#4a4d5c">${request.groupSize} participants · ${request.audienceType}<br/>Preferred: ${request.preferredDates}</p>
<p>Warm regards,<br/>iqcommune team<br/><a href="mailto:hello@iqcommune.com">hello@iqcommune.com</a></p>
</div>`,
  };
}
...
export function sessionConfirmationEmail(
  ...
  const first = practitionerName.split(" ")[0];
  return {
    subject: `iqcommune — Session Confirmation: ${session.refCode}`,
    htmlContent: `<div style="${BASE}">
<p>Hi ${first},</p>
...
<div style="text-align:center;margin:2rem 0">
  <a href="${session.consentUrl}" style="${GOLD_BTN}">Provide Digital Consent →</a>
</div>
