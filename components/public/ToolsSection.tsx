...
function WidgetHeader({ title }: { title: string }) {
  return (
    <div style={{ padding: "9px 13px", borderBottom: "1px solid rgba(201,152,42,0.12)", display: "flex", justifyContent: "space-between", alignItems: "center" }}>
      <span style={{ fontSize: 11, fontWeight: 500, color: VAL_COLOR }}>{title}</span>
      <span style={{ fontSize: 9, padding: "2px 7px", borderRadius: 20, background: "rgba(201,152,42,0.12)", color: "var(--gold)", border: "0.5px solid rgba(201,152,42,0.3)" }}>Try it</span>
    </div>
  );
}

function CtrlRow({ label, valueStr, min, max, step, value, onChange }: {
  label: string; valueStr: string; min: number; max: number; step: number; value: number;
  onChange: (v: number) => void;
}) {
  return (
    <div style={{ marginBottom: 9 }}>
      <div style={{ fontSize: 9.5, textTransform: "uppercase", letterSpacing: "0.07em", color: LBL_COLOR, marginBottom: 3, display: "flex", justifyContent: "space-between" }}>
        <span>{label}</span>
        <span style={{ fontSize: 11.5, fontWeight: 500, color: VAL_COLOR, textTransform: "none", letterSpacing: 0 }}>{valueStr}</span>
      </div>
      <input type="range" min={min} max={max} step={step} value={value}
        onChange={e => onChange(parseFloat(e.target.value))}
        style={{ width: "100%", accentColor: "var(--gold)", background: "transparent", cursor: "pointer" }}
      />
    </div>
  );
}
...
function Box({ label, value, cls }: { label: string; value: string; cls?: string }) {
  const color = cls === "pos" ? "#4ec994" : cls === "neg" ? "#e06c75" : cls === "warn" ? "var(--gold)" : VAL_COLOR;
  return (
    <div style={{ background: BOX_BG, borderRadius: 5, padding: "7px 9px" }}>
      <div style={{ fontSize: 9, textTransform: "uppercase", letterSpacing: "0.06em", color: BOX_LBL_C, marginBottom: 2 }}>{label}</div>
      <div style={{ fontSize: 13, fontWeight: 500, color, fontVariantNumeric: "tabular-nums" }}>{value}</div>
    </div>
  );
}

function Flag({ type, text }: { type: "good" | "warn" | "bad" | "none"; text: string }) {
  if (!text) return null;
  const style: React.CSSProperties =
    type === "good" ? { background: "rgba(78,201,148,0.1)", color: "#4ec994", border: "0.5px solid rgba(78,201,148,0.25)" } :
    type === "warn" ? { background: "rgba(201,152,42,0.1)", color: "var(--gold)", border: "0.5px solid rgba(201,152,42,0.25)" } :
    type === "bad"  ? { background: "rgba(224,108,117,0.1)", color: "#e06c75", border: "0.5px solid rgba(224,108,117,0.25)" } :
    {};
  return <div style={{ marginTop: 8, padding: "6px 9px", borderRadius: 5, fontSize: 11, lineHeight: 1.4, ...style }}>{text}</div>;
}
...
function BudgetChecker() {
  ...
  const tenYrCorpus = annualSave * ((Math.pow(1.10, 10) - 1) / 0.10 * 1.10);
  return (
    <div style={{ background: WIDGET_BG, border: WIDGET_BOR, borderRadius: 8, overflow: "hidden", flex: 1 }}>
      <WidgetHeader title="Budget Checker" />
      <div style={{ padding: "12px 13px" }}>
        <CtrlRow label="Monthly take-home" valueStr={fmt(income)} min={15000} max={300000} step={5000} value={income} onChange={setIncome} />
        <div style={{ display: "flex", height: 6, borderRadius: 3, overflow: "hidden", gap: 1, marginTop: 8 }}>
          <div style={{ width: "50%", background: "#c9982a", height: "100%" }} />
          <div style={{ width: "30%", background: "#4a7abe", height: "100%" }} />
          <div style={{ width: "20%", background: "#4ec994", height: "100%" }} />
        </div>
...
function RetirementCorpus() {
  ...
  const rm = 0.07 / 12, gm = 0.06 / 12, nm = 20 * 12;
  const corpus = futureMonthly * (1 - Math.pow((1 + gm) / (1 + rm), nm)) / (rm - gm);
  ...
  return (
    <div style={{ background: WIDGET_BG, border: WIDGET_BOR, borderRadius: 8, overflow: "hidden", flex: 1 }}>
      ...
      <div style={{ padding: "12px 13px" }}>
        <CtrlRow label="Current age" valueStr={String(age)} min={22} max={55} step={1} value={age} onChange={v => setAge(Math.round(v))} />
        <CtrlRow label="Monthly savings" valueStr={fmt(sav)} min={5000} max={150000} step={5000} value={sav} onChange={setSav} />
        <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 5 }}>
          <Box label="Corpus needed" value={fmt(corpus)} cls="warn" />
          <Box label="SIP required" value={fmt(sipNeeded)} cls="pos" />
        </div>
        <div style={{ fontSize: 10.5, fontStyle: "italic", color: "rgba(255,255,255,0.35)", lineHeight: 1.5, marginTop: 8 }}>
          {yearsLeft} years to retirement. Assumed 6% inflation, 7% withdrawal-phase return, 20-year retirement window.
        </div>
      </div>
    </div>
  );
...
function PEValuation() {
  ...
  const verdict = pe === null ? null : pe < 15 ? "Cheap" : pe <= 28 ? "Fair" : "Stretched";
  const vType  = pe === null ? "none" : pe < 15 ? "good" : pe <= 28 ? "warn" : "bad";
  const vColor = pe === null ? VAL_COLOR : pe < 15 ? "#4ec994" : pe <= 28 ? "var(--gold)" : "#e06c75";
  const flagText = pe === null ? "" :
    pe < 15  ? `P/E of ${pe.toFixed(1)}× is below 15 — trading at a discount to the market. Worth investigating why.` :
    pe <= 28 ? `P/E of ${pe.toFixed(1)}× is in line with market. Valuation is not the edge here — earnings quality is.` :
  ...
  return (
    <div style={{ background: WIDGET_BG, border: WIDGET_BOR, borderRadius: 8, overflow: "hidden", flex: 1 }}>
      ...
      <div style={{ padding: "12px 13px" }}>
        <CtrlRow label="Market price (₹)" valueStr={`₹${price.toLocaleString("en-IN")}`} min={10} max={5000} step={10} value={price} onChange={v => setPrice(Math.round(v))} />
        <CtrlRow label="EPS (₹)" valueStr={`₹${eps}`} min={1} max={500} step={1} value={eps} onChange={v => setEps(Math.round(v))} />
        <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr 1fr", gap: 5 }}>
          <Box label="P/E Ratio" value={pe !== null ? pe.toFixed(1) + "×" : "N/A"} />
          <Box label="Market avg" value="22×" />
          <div style={{ background: BOX_BG, borderRadius: 5, padding: "7px 9px" }}>
            <div style={{ fontSize: 9, textTransform: "uppercase", letterSpacing: "0.06em", color: BOX_LBL_C, marginBottom: 2 }}>Verdict</div>
            <div style={{ fontSize: 13, fontWeight: 500, color: vColor, fontVariantNumeric: "tabular-nums" }}>{verdict ?? "—"}</div>
          </div>
        </div>
        <Flag type={vType as "good"|"warn"|"bad"|"none"} text={flagText} />
      </div>
    </div>
  );
}
...
function DebtComparator() {
  ...
  return (
    <div style={{ background: WIDGET_BG, border: WIDGET_BOR, borderRadius: 8, overflow: "hidden", flex: 1 }}>
      ...
      <div style={{ padding: "12px 13px" }}>
        <CtrlRow label="Amount (₹)" valueStr={fmt(amt)} min={10000} max={1000000} step={10000} value={amt} onChange={setAmt} />
        <CtrlRow label="Tenure (years)" valueStr={`${yrs} yr${yrs > 1 ? "s" : ""}`} min={1} max={10} step={1} value={yrs} onChange={v => setYrs(Math.round(v))} />
        <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr 1fr", gap: 5 }}>
        ...
        <div style={{ fontSize: 10.5, fontStyle: "italic", color: "rgba(255,255,255,0.35)", lineHeight: 1.5, marginTop: 8 }}>
          Assumes 30% tax slab. Illustrative only — actual returns vary with rate cycles and tax slab.
        </div>
      </div>
    </div>
  );
...
function PortfolioScorecard() {
  ...
  const rawScore = Math.max(0, 10 - (Math.abs(eq - tgt.eq) + Math.abs(de - tgt.de)) / 5);
  const score = Math.round(rawScore * 10) / 10;
  const scoreType = score >= 8 ? "pos" : score >= 5 ? "warn" : "neg";
  const flagType: "good"|"warn"|"bad" = score >= 8 ? "good" : score >= 5 ? "warn" : "bad";
  const flagText = score >= 8
    ...
    <div style={{ background: WIDGET_BG, border: WIDGET_BOR, borderRadius: 8, overflow: "hidden", flex: 1 }}>
      ...
      <div style={{ padding: "12px 13px" }}>
        <div style={{ display: "flex", gap: 5, marginBottom: 9 }}>
          {T5_PROFILES.map((p, i) => (
            <button key={p.label} onClick={() => handleProfile(i)}
              style={{ fontSize: 10, padding: "3px 9px", borderRadius: 20, cursor: "pointer",
                ...
                background: i === profile ? "rgba(201,152,42,0.08)" : "transparent",
                transition: "all 0.15s" }}>
              {p.label}
            </button>
          ))}
        </div>
        <CtrlRow label="Equity %" valueStr={`${eq}%`} min={0} max={100} step={5} value={eq} onChange={v => setEq(Math.round(v))} />
        <CtrlRow label="Debt %" valueStr={`${de}%`} min={0} max={100} step={5} value={de} onChange={v => setDe(Math.round(v))} />
        <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 5 }}>
          <Box label="Balance score" value={`${score.toFixed(1)}/10`} cls={scoreType} />
...
function SipVisualiser() {
  ...
  vals.forEach((v, i) => bars.push({ h: Math.max(4, Math.round((v / maxVal) * 36)), isLast: i === vals.length - 1 }));

  return (
    <div style={{ background: WIDGET_BG, border: WIDGET_BOR, borderRadius: 8, overflow: "hidden", flex: 1 }}>
      <WidgetHeader title="SIP Growth" />
      <div style={{ padding: "12px 13px" }}>
        <CtrlRow label="Monthly SIP" valueStr={fmt(sip)} min={1000} max={100000} step={1000} value={sip} onChange={setSip} />
        <CtrlRow label="Years" valueStr={String(yrs)} min={3} max={30} step={1} value={yrs} onChange={v => setYrs(Math.round(v))} />
        <CtrlRow label="Return %" valueStr={`${ret}%`} min={6} max={18} step={0.5} value={ret} onChange={setRet} />
        <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr 1fr", gap: 5 }}>
        ...
        <div style={{ display: "flex", alignItems: "flex-end", gap: 2, height: 40, marginTop: 8 }}>
          {bars.map((b, i) => (
            <div key={i} style={{ flex: 1, height: b.h, borderRadius: "2px 2px 0 0",
              background: b.isLast ? "#c9982a" : "rgba(201,152,42,0.35)", minHeight: 3 }} />
          ))}
        </div>
      </div>
    </div>
...
const TOOLS_DATA = [
  ...
  { module: "Investment Solutions & Portfolio Strategies",  name: "SIP Growth Visualiser",          desc: "Monthly SIP + years + return rate. Watch your corpus build year by year.",                                                       Widget: SipVisualiser    },
] as const;
    ...
    <section style={{ background: "#14161d", padding: "4rem 2rem" }}>
      ...
      <div style={{ maxWidth: 1100, margin: "0 auto" }}>
        ...
        <div style={{ textAlign: "center", marginBottom: "1rem" }}>
          <span style={{ display: "inline-flex", alignItems: "center", gap: 6, fontSize: 11, fontWeight: 600, letterSpacing: "0.12em", textTransform: "uppercase", color: "#f0c84a", background: "rgba(201,152,42,0.15)", border: "1px solid rgba(201,152,42,0.3)", padding: "5px 14px", borderRadius: 100 }}>
            Tools &amp; Calculators
          </span>
        </div>

        <h2 style={{ fontSize: "clamp(26px,3.8vw,40px)", fontWeight: 600, lineHeight: 1.2, letterSpacing: "-0.01em", textAlign: "center", color: "#ffffff", marginBottom: "1rem" }}>
          Try the numbers before you attend.
        </h2>
        <p style={{ fontSize: 16, color: "rgba(255,255,255,0.5)", textAlign: "center", maxWidth: 560, margin: "0 auto 3rem" }}>
          Six live calculators — one per module. Built on the same frameworks the sessions use. No sign-up, no data stored.
        </p>
