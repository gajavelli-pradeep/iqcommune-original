...
const SLIDES = [
  { label: "From the back of the room", sub: "Trainer in focus, audience visible", tag: "Foundations", city: null },
  { label: "Audience in focus", sub: "Trainer's perspective, room engaged", tag: "Equity Investing", city: "Mumbai" },
  { label: "Group photo", sub: "End of session", tag: "Portfolio Strategy", city: null },
  { label: "Candid — mid session", sub: "Numbers being worked through", tag: "Retirement Planning", city: "Bengaluru" },
  { label: "Front-left corner view", sub: "Full room, wide angle", tag: "Asset Allocation", city: null },
  { label: "Candid — Q&A moment", sub: "Participant raising a question", tag: "Debt Investing", city: "Pune" },
  { label: "Front-right corner view", sub: "Trainer and board visible", tag: "Foundations", city: "Delhi" },
] as const;
...
export function GallerySection() {
  ...
  const goTo = useCallback((idx: number) => {
    ...
    setCurrent(next);
    if (trackRef.current) {
      const offset = Math.min(next, maxOffset) * STEP;
      ...
      >
        ...
        <div style={{ display: "flex", gap: 6 }}>
          {SLIDES.map((_, i) => (
            <button
              key={i}
              onClick={() => { if (timerRef.current) clearTimeout(timerRef.current); goTo(i); scheduleAuto(); }}
              aria-label={`Go to slide ${i + 1}`}
              style={{
                width: current === i ? 18 : 6,
                height: 6,
                borderRadius: current === i ? 3 : "50%",
                background: current === i ? "var(--gold)" : "rgba(255,255,255,0.15)",
                border: "none",
                cursor: "pointer",
                padding: 0,
